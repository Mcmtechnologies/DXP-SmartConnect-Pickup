...
namespace DXP.SmartConnectPickup.Common.WebApi
{
    public class WebApiClient
    {
        ...
        public Task<T> GetAsync<T>(string description, Uri requestUri, CancellationToken cancellationToken, string applicableProvider, bool ignoreChecking = false)
        {
            return CallApiAsync<T>(description, client => CheckForWebException(description, client.GetAsync(requestUri, cancellationToken), ignoreChecking), applicableProvider);
        }
            ...
            bool ignoreChecking = false)
        {
            ...
            return CallApiAsync<TResponse>(description,
                client => CheckForWebException(description, client.PostAsync(requestUri, serializedRequestContent, cancellationToken), ignoreChecking),
                applicableProvider);
        }
        ...
        {
            return CallApiAsync<TResponse>(description,
                client => CheckForWebException(description, client.PutAsync(requestUri, SerializeAsJsonString(requestContent), cancellationToken), ignoreChecking), applicableProvider);
        }
            ...
            bool ignoreChecking = false)
        {
            return CallApiAsync<T>(description,
                client => CheckForWebException(description, client.DeleteAsync(requestUri, cancellationToken), ignoreChecking), applicableProvider);
        }

        public Task<T> SendAsync<T>(string description,
            HttpRequestMessage requestMessage,
            CancellationToken cancellationToken,
            string applicableProvider,
            bool ignoreChecking = false)
        {
            return CallApiAsync<T>(description, client => CheckForWebException(description, client.SendAsync(requestMessage, cancellationToken), ignoreChecking), applicableProvider);
        }
        ...
        private async Task<T> CallApiAsync<T>(string description, Func<HttpClient, Task<HttpResponseMessage>> apiCall, string applicableProvider)
        {
            try
            {
                ...
                using (HttpResponseMessage response = await apiCall(_httpClient))
                {
                    ...
                    timer.Stop();
                    _logger.LogInformation($"apiCall ended after {timer.ElapsedMilliseconds}ms");
                    ...
                    if (value is BaseHttpResponse)
                    {
                        (value as BaseHttpResponse).Response = response;
                    }
                }
